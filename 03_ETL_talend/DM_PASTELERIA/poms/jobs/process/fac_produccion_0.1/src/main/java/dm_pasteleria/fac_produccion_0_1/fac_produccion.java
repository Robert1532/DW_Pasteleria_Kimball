...
package dm_pasteleria.fac_produccion_0_1;
 ...
 * @status
 */
public class fac_produccion implements TalendJob {
	...
	private final String jobName = "fac_produccion";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					fac_produccion.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(fac_produccion.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class fact_produccionStruct implements routines.system.IPersistableRow<fact_produccionStruct> {
		final static byte[] commonByteArrayLock_DM_PASTELERIA_fac_produccion = new byte[0];
		static byte[] commonByteArray_DM_PASTELERIA_fac_produccion = new byte[0];
		...
		public BigDecimal ID_EMPLEADO;

		public BigDecimal getID_EMPLEADO() {
			return this.ID_EMPLEADO;
		}

		public BigDecimal CANTIDAD_PROD;

		public BigDecimal getCANTIDAD_PROD() {
			return this.CANTIDAD_PROD;
		}

		public BigDecimal COSTO_MP;

		public BigDecimal getCOSTO_MP() {
			return this.COSTO_MP;
		}

		public BigDecimal MERMA;
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DM_PASTELERIA_fac_produccion.length) {
					if (length < 1024 && commonByteArray_DM_PASTELERIA_fac_produccion.length == 0) {
						commonByteArray_DM_PASTELERIA_fac_produccion = new byte[1024];
					} else {
						commonByteArray_DM_PASTELERIA_fac_produccion = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DM_PASTELERIA_fac_produccion, 0, length);
				strReturn = new String(commonByteArray_DM_PASTELERIA_fac_produccion, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_DM_PASTELERIA_fac_produccion, 0, length);
				strReturn = new String(commonByteArray_DM_PASTELERIA_fac_produccion, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DM_PASTELERIA_fac_produccion) {
				...
				try {
					...
					this.ID_EMPLEADO = (BigDecimal) dis.readObject();

					this.CANTIDAD_PROD = (BigDecimal) dis.readObject();

					this.COSTO_MP = (BigDecimal) dis.readObject();

					this.MERMA = (BigDecimal) dis.readObject();
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeObject(this.ID_EMPLEADO);

				// BigDecimal

				dos.writeObject(this.CANTIDAD_PROD);

				// BigDecimal

				dos.writeObject(this.COSTO_MP);

				// BigDecimal

				dos.writeObject(this.MERMA);
		...
		public String toString() {
			...
			sb.append(",CANTIDAD_PROD=" + String.valueOf(CANTIDAD_PROD));
			sb.append(",COSTO_MP=" + String.valueOf(COSTO_MP));
			sb.append(",MERMA=" + String.valueOf(MERMA));
		 ...
		 * Compare keys
		 */
		public int compareTo(fact_produccionStruct other) {
	...
	public void tWaitForFile_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				fact_produccionStruct fact_produccion = new fact_produccionStruct();
				...
				class Util_tWaitForFile_1 {
					...
					void setFiles() {
						String directoryt = "C:/Users/rober/Desktop/paste,eria/Produccion/Pendientes";
				...
				while (true) {
					...
					if (execStat) {
						runStat.updateStatOnConnection("fact_produccion", 3, 0);
					}
					...
					if (execStat) {
						runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "fact_produccion");
					}
					...
					final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
							"enc:routine.encryption.key.v1:w686Mzhbtu/n0jj5GPAfHJL3w+u3C49fcXHCaZSnf+Zh9G1TjQ60Zw==");
					...
					if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
						tableName_tDBOutput_1 = ("FAC_PRODUCCION");
					} else {
						tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "." + ("FAC_PRODUCCION");
					}
					String insert_tDBOutput_1 = "INSERT INTO " + tableName_tDBOutput_1
							+ " (ID_TIEMPO,ID_PRODUCTO,ID_SUCURSAL,ID_EMPLEADO,CANTIDAD_PROD,COSTO_MP,MERMA,FECHA_CARGA,SISTEMA_CARGA) VALUES (?,?,?,?,?,?,?,?,?)";
...
// # Outputs initialization
					fact_produccionStruct fact_produccion_tmp = new fact_produccionStruct();
// ###############################
							...
							try {
								...
								if (temp.length() > 0) {
									...
									try {
										...
										row1.id_empleado = ParserUtils.parseTo_BigDecimal(temp);
										...
										rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
												"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"id_empleado", "row1", temp, ex_tFileInputDelimited_1),
												ex_tFileInputDelimited_1));
									}

								} else {

									row1.id_empleado = null;
								...
								if (temp.length() > 0) {
									...
									try {
										...
										row1.cantidad_prod = ParserUtils.parseTo_BigDecimal(temp);
										...
										rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
												"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"cantidad_prod", "row1", temp, ex_tFileInputDelimited_1),
												ex_tFileInputDelimited_1));
									}

								} else {

									row1.cantidad_prod = null;
								...
								if (temp.length() > 0) {
									...
									try {
										...
										row1.costo_mp = ParserUtils.parseTo_BigDecimal(temp);
										...
										rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
												"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"costo_mp", "row1", temp, ex_tFileInputDelimited_1),
												ex_tFileInputDelimited_1));
									}
								...
								if (temp.length() > 0) {
									...
									try {
										...
										row1.merma = ParserUtils.parseTo_BigDecimal(temp);
										...
										rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
												"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"merma", "row1", temp, ex_tFileInputDelimited_1),
												ex_tFileInputDelimited_1));
									}
...
// Start of branch "row1"
							if (row1 != null) {
								...
								{ // start of Var scope
									...
									fact_produccion = null;

// # Output table : 'fact_produccion'
									fact_produccion_tmp.ID_TIEMPO = row1.id_tiempo;
									fact_produccion_tmp.ID_PRODUCTO = row1.id_producto;
									fact_produccion_tmp.ID_SUCURSAL = row1.id_sucursal;
									fact_produccion_tmp.ID_EMPLEADO = row1.id_empleado;
									fact_produccion_tmp.CANTIDAD_PROD = row1.cantidad_prod;
									fact_produccion_tmp.COSTO_MP = row1.costo_mp;
									fact_produccion_tmp.MERMA = row1.merma;
									fact_produccion_tmp.FECHA_CARGA = row1.fecha_carga;
									fact_produccion_tmp.SISTEMA_CARGA = row1.sistema_carga;
									fact_produccion = fact_produccion_tmp;
// ###############################
								 ...
								 */
// Start of branch "fact_produccion"
								if (fact_produccion != null) {
									...
									pstmt_tDBOutput_1.setBigDecimal(1, fact_produccion.ID_TIEMPO);

									pstmt_tDBOutput_1.setBigDecimal(2, fact_produccion.ID_PRODUCTO);

									pstmt_tDBOutput_1.setBigDecimal(3, fact_produccion.ID_SUCURSAL);

									pstmt_tDBOutput_1.setBigDecimal(4, fact_produccion.ID_EMPLEADO);

									pstmt_tDBOutput_1.setBigDecimal(5, fact_produccion.CANTIDAD_PROD);

									pstmt_tDBOutput_1.setBigDecimal(6, fact_produccion.COSTO_MP);

									pstmt_tDBOutput_1.setBigDecimal(7, fact_produccion.MERMA);

									if (fact_produccion.FECHA_CARGA == null) {
										pstmt_tDBOutput_1.setNull(8, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(8, fact_produccion.FECHA_CARGA);
									}

									if (fact_produccion.SISTEMA_CARGA == null) {
										pstmt_tDBOutput_1.setNull(9, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(9, fact_produccion.SISTEMA_CARGA);
									}
...
// ###############################      
					...
					if (execStat) {
						runStat.updateStat(resourceMap, iterateId, 2, 0, "fact_produccion");
					}
	...
	public void tFileCopy_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String desDirName_tFileCopy_1 = "C:/Users/rober/Desktop/paste,eria/Produccion/Procesados";
	...
	public static void main(String[] args) {
		final fac_produccion fac_produccionClass = new fac_produccion();

		int exitCode = fac_produccionClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = fac_produccion.class.getClassLoader()
					.getResourceAsStream("dm_pasteleria/fac_produccion_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = fac_produccion.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : fac_produccion");
		}
...
/************************************************************************************************
 * 84518 characters generated by Talend Open Studio for Data Integration on the
 * 6 de diciembre de 2025, 01:51:07 BOT
 ************************************************************************************************/